...
As a simple example, suppose we want to simulate `n = 10` curves from a zero-mean Gaussian process covariance function given by $C(x_p, x_q) = \min(x_p, x_q)$ (namely Brownian Motion). This can be achieved by using the `compute_cov_matrix()` function from `GPcov` package, and the `mvrnorm()` function from the `MASS` package.
...
Alternatively, suppose we want to simulate a Matérn process in $\mathbb{R}^2$. This can be done in the following way.
